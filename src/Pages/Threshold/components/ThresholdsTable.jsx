...
const ThresholdsTable = ({ thresholds, selectedRow, onSelectedChange }) => {
  const columns = useMemo(() => [
    { name: 'SL', selector: (row, i) => i + 1, width: '70px' },
    { name: 'THRESHOLD(S) NAME', selector: row => row.name, sortable: true },
    { name: 'THRESHOLD START', selector: row => row.start, sortable: true, right: true },
    { name: 'THRESHOLD END', selector: row => row.end, sortable: true, right: true },
    { name: 'DURATION (SEC)', selector: row => row.duration, sortable: true, right: true },
  ], []);
